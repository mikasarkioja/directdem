...
describe("Espoo-vahti Scraper integraatiotesti", () => {
  ...
  it("fetchEspooDynastyLinks hakee ja parsii linkit oikein", async () => {
    ...
    (axios.get as any).mockResolvedValue({ data: mockHtml, status: 200 });
  ...
  it("fetchMeetingItems löytää asiat ja PDF-linkit", async () => {
    ...
    (axios.get as any).mockResolvedValue({
      data: mockMeetingHtml,
      status: 200,
    });

    const items = await fetchMeetingItems(
      "https://espoo.oncloudos.com/cgi/DREQUEST.PHP?page=meeting_minutes&id=123",
    );

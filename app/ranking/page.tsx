          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xs font-black uppercase tracking-widest text-slate-400">Kurinalaisin</h3>
              ...
              <div className="space-y-4">
                {data.leaderboards.disciplined.map((p, i) => (
                  <div key={p.name} className="flex items-center justify-between group">
                    <div className="flex items-center gap-3">
                      <span className="w-6 h-6 rounded-lg bg-slate-100 flex items-center justify-center text-[10px] font-black text-slate-400 group-hover:bg-emerald-500 group-hover:text-white transition-colors">{i+1}</span>
                      <span className="font-black uppercase text-sm text-slate-700">{p.name}</span>
                    </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xs font-black uppercase tracking-widest text-slate-400">Muuttuvin Linja</h3>
                  <p className="text-sm font-black uppercase text-slate-900">Pivot Score poikkeama</p>
                </div>
              </div>
              <div className="space-y-4">
                {data.leaderboards.flipFlops.map((p, i) => (
                  <div key={p.name} className="flex items-center justify-between group">
                    <div className="flex items-center gap-3">
                      <span className="w-6 h-6 rounded-lg bg-slate-100 flex items-center justify-center text-[10px] font-black text-slate-400 group-hover:bg-amber-500 group-hover:text-white transition-colors">{i+1}</span>
                      <span className="font-black uppercase text-sm text-slate-700">{p.name}</span>
                    </div>
                    <span className="font-black text-amber-600">+{p.score}%</span>
                  </div>
                ))}
              </div>
            </motion.div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xs font-black uppercase tracking-widest text-slate-400">Hallitsijat</h3>
                  <p className="text-sm font-black uppercase text-slate-900">Topic Ownership</p>
                </div>
              </div>
              <div className="space-y-4">
                {data.leaderboards.owners.slice(0, 3).map((o, i) => (
                  <div key={o.category} className="flex items-center justify-between group">
                    <div className="flex items-center gap-3">
                      <span className="font-black uppercase text-[10px] text-slate-400 w-24">{o.category}</span>
                      <span className="font-black uppercase text-sm text-slate-700">{o.party}</span>
                    </div>
            ...
            <div className="bg-white border border-slate-200 rounded-[3rem] p-10 shadow-sm space-y-8">
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Etäisyys eduskunnan keskipisteestä</p>
                </div>
            ...
            <div className="bg-white border border-slate-200 rounded-[3rem] p-10 shadow-sm space-y-8">
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Yhtenäisyys suhteessa poikkeamaan</p>
                </div>

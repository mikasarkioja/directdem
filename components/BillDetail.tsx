...
export default function BillDetail({ bill, onClose }: BillDetailProps) {
  ...
  useEffect(() => {
    ...
    fetchEnhancedMunicipalProfile(billIdForEnhanced, "parliament").then(
      setEnhancedData,
    );
  ...
  const handleRegenerate = async () => {
    ...
    try {
      ...
      if (result.success && result.summary) {
        ...
        const billIdForEnhanced = bill.parliamentId || bill.id;
        const freshEnhanced = await fetchEnhancedMunicipalProfile(
          billIdForEnhanced,
          "parliament",
        );
    ...
    >
      ...
      >
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <p className="text-slate-500 text-[9px] font-black uppercase tracking-[0.2em]">
                  {bill.parliamentId || "Eduskunta"}
                </p>
                <span className="w-1 h-1 rounded-full bg-slate-700" />
                <p className="text-purple-500 text-[9px] font-black uppercase tracking-[0.2em]">
                  Eduskuntavahti
                </p>
              </div>
            </div>
        ...
        <div className="p-6 md:p-12 space-y-12">
          ...
          >
            {(() => {
              const passProbability =
                enhancedData?.forecast_metrics?.pass_probability ??
                enhancedData?.analysis_data?.analysis_depth
                  ?.strategy_match_score ??
                politicalForPercent;
              const lobbyIndex =
                enhancedData?.forecast_metrics?.lobbying_index ??
                enhancedData?.analysis_data?.analysis_depth?.friction_index ??
                Math.max(0, Math.min(100, alerts.length * 12 + 25));

              return (
                <>
                  <div className="p-6 rounded-2xl border border-emerald-500/20 bg-slate-950/50 space-y-3">
                    ...
                    <div
                      ...
                      aria-valuemin={0}
                      aria-valuemax={100}
                      aria-valuenow={Math.round(passProbability)}
                      aria-label="Lapimenon todennakoisyys"
                    >
                      <div
                        className={`${passProbability >= 55 ? "bg-emerald-500" : "bg-rose-500"} h-full`}
                        style={{
                          width: `${Math.max(0, Math.min(100, passProbability))}%`,
                        }}
                      />
                    </div>
                    <p className="text-sm font-bold text-slate-200">
                      Lapimenoarvio: {Math.round(passProbability)}%
                    </p>
                  </div>

                  <div className="p-6 rounded-2xl border border-amber-500/20 bg-slate-950/50 space-y-3">
                    ...
                    <div
                      ...
                      aria-valuemin={0}
                      aria-valuemax={100}
                      aria-valuenow={Math.round(lobbyIndex)}
                      aria-label="Lobbausindeksi"
                    >
                      <div
                        className={`${lobbyIndex >= 70 ? "bg-orange-500" : lobbyIndex >= 45 ? "bg-amber-500" : "bg-emerald-500"} h-full`}
                        style={{
                          width: `${Math.max(0, Math.min(100, lobbyIndex))}%`,
                        }}
                      />
                    </div>
                    <p className="text-sm font-bold text-slate-200">
                      Lobbausindeksi: {Math.round(lobbyIndex)}/100
                    </p>
                  </div>
                </>
              );
          ...
          {/* Tertiary focus: engagement as secondary CTA */}
          <section className="flex flex-wrap items-center justify-between gap-3 p-4 rounded-2xl border border-purple-500/20 bg-purple-500/5">
            <p className="text-xs text-slate-300 font-medium">
              Haluatko osallistua? Varjo-aanestys on nyt toissijainen toiminto
              taman analyysin alla.
            </p>
            <Link
              href={`/arena?billId=${bill.id}`}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-purple-500/30 bg-purple-500/10 text-[10px] font-black uppercase tracking-widest text-purple-300 hover:bg-purple-500/20 transition-all"
            >
              Avaa varjo-aanestys
          ...
          {/* Advanced tools moved behind explicit deep-analysis toggle */}
          {enhancedData?.analysis_data?.analysis_depth && (
            <details className="rounded-[2rem] border border-white/10 bg-slate-950/30 p-6 group">
              <summary className="cursor-pointer list-none flex items-center justify-between">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">
                    Syvaanalyysi (Advanced)
                  </p>
                  <p className="text-sm text-slate-300 mt-1">
                    Avaa tutkijatyokalut, talousmallit ja strateginen
                    datanakyma.
                  </p>
                </div>
              ...
              <motion.div
                ...
                className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-8 p-8 bg-slate-950/50 border border-purple-500/20 rounded-[2.5rem] shadow-inner"
              >
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="p-6 bg-white/5 rounded-[2rem] border border-white/5 relative overflow-hidden group">
                      ...
                      <p className="text-[8px] font-black uppercase text-slate-500 mb-1 tracking-widest">
                        Kustannusarvio (Kokoluokka)
                      </p>
                      <p className="text-2xl font-black text-white mb-1">
                        {enhancedData.analysis_data.analysis_depth
                          .economic_impact.total_cost_estimate > 0
                          ? new Intl.NumberFormat("fi-FI", {
                              ...
                              enhancedData.analysis_data.analysis_depth
                                .economic_impact.total_cost_estimate,
                            )
                          : "Selvitetään..."}
                      </p>

                      {/* Per Citizen Calculation */}
                      {enhancedData.analysis_data.analysis_depth.economic_impact
                        ...
                        <div className="flex items-center gap-2 mb-4">
                          <div className="px-2 py-0.5 bg-purple-500/20 rounded text-[9px] font-bold text-purple-400 border border-purple-500/20">
                            {new Intl.NumberFormat("fi-FI", {
                              ...
                              enhancedData.analysis_data.analysis_depth
                                .economic_impact.total_cost_estimate / 5600000,
                            )}{" "}
                            / kansalainen
                          </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="p-4 bg-white/5 rounded-2xl border border-white/5">
                        ...
                        <p className="text-[10px] font-bold text-slate-300">
                          {
                            enhancedData.analysis_data.analysis_depth
                              .economic_impact.funding_source
                          }
                        </p>
                      </div>
                      <div className="p-4 bg-white/5 rounded-2xl border border-white/5">
                        ...
                        <p className="text-[10px] font-bold text-slate-300">
                          {
                            enhancedData.analysis_data.analysis_depth
                              .economic_impact.budget_alignment
                          }
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-white/5 rounded-2xl border border-white/5">
                      ...
                      <p className="text-xs font-bold text-slate-200">
                        {
                          enhancedData.analysis_data.analysis_depth
                            .strategic_analysis.primary_driver
                        }
                      </p>
                    </div>
                    ...
                    <div className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5">
                      <span className="text-[8px] font-black uppercase text-slate-500">
                        Hallitusohjelma-yhteensopivuus
                      </span>
                      <div className="flex items-center gap-2">
                        <div className="w-20 h-1.5 bg-slate-900 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${enhancedData.analysis_data.analysis_depth.strategic_analysis.strategy_match_score}%`,
                            }}
                          />
                        </div>
                        <span className="text-[10px] font-black text-blue-400">
                          {
                            enhancedData.analysis_data.analysis_depth
                              .strategic_analysis.strategy_match_score
                          }
                          %
                        </span>
                      </div>
                ...
                <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {enhancedData.analysis_data.analysis_depth.social_equity.winners.map(
                        (w: string, i: number) => (
                          <span
                            key={i}
                            className="px-3 py-1 bg-emerald-500/10 text-emerald-400 text-[9px] font-bold rounded-full border border-emerald-500/20"
                          >
                            {w}
                          </span>
                        ),
                  ...
                  <div className="space-y-3">
                    <p className="text-[8px] font-black uppercase text-rose-500 tracking-widest">
                      Häviäjät
                    </p>
                    <div className="flex flex-wrap gap-2">
                      {enhancedData.analysis_data.analysis_depth.social_equity.losers.map(
                        (l: string, i: number) => (
                          <span
                            key={i}
                            className="px-3 py-1 bg-rose-500/10 text-rose-400 text-[9px] font-bold rounded-full border border-rose-500/20"
                          >
                            {l}
                          </span>
                        ),
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-8">
              ...
              <div className="bg-slate-900 border border-purple-500/20 p-10 rounded-[2rem] shadow-[0_0_30px_rgba(168,85,247,0.1)] h-full flex flex-col justify-center">
                ...
                <VoteButton
                  billId={bill.id}
                  onVoteChange={() => getVoteStats(bill.id).then(setVoteStats)}
                />
              </div>
            </div>
          </div>
            ...
            <div className="space-y-8 pt-16 border-t border-white/5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {alerts.slice(0, 6).map((alert) => (
                  <div
                    key={alert.id}
                    className="bg-slate-800/50 border border-white/5 p-6 rounded-[1.5rem] space-y-6 hover:border-amber-500/30 transition-all"
                  >
                    <div className="flex justify-between items-start">
                      <div className="flex items-center gap-2">
                        <div
                          className={`w-2 h-2 rounded-full ${alert.severity === "high" ? "bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.6)]" : "bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.6)]"}`}
                        />
                        <span
                          className={`text-[10px] font-black uppercase tracking-widest ${alert.severity === "high" ? "text-rose-400" : "text-amber-400"}`}
                        >
                          {alert.severity === "high"
                            ? "Takinkääntö"
                            : "Poikkeama"}
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden">
                          <div
                            className={`h-full ${alert.promise_value > 0 ? "bg-emerald-500" : "bg-rose-500"}`}
                            style={{ width: "100%" }}
                          />
                        </div>
                      </div>
                      <div className="flex flex-col gap-1">
                        ...
                        <div
                          className={`px-3 py-2 rounded-xl text-center font-black uppercase text-xs border ${alert.vote_type === "jaa" ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : "bg-rose-500/10 border-rose-500/20 text-rose-400"}`}
                        >
                          {alert.vote_type === "jaa"
                            ? "Äänesti: JAA"
                            : "Äänesti: EI"}

...
function generateDistrictStats(
  ...
  const baseParliament = totalSeats > 0 ? (forSeats / totalSeats) * 100 : 50;
  ...
  if (titleLower.includes("vero") || titleLower.includes("talous")) {
    ...
    titleLower.includes("luonto") ||
    titleLower.includes("metsä") ||
    titleLower.includes("maatalous")
  ) {
    // Rural districts usually favor these more if it's about usage, or less if it's about restriction
    regionalVariation *= 1.5;
  } else if (
    titleLower.includes("alkoholi") ||
    titleLower.includes("vapaa-aika")
  ) {
  ...
  const citizenVote = Math.max(
    5,
    Math.min(95, baseCitizen + regionalVariation + noise),
  );
  ...
  const parliamentVote = Math.max(
    5,
    Math.min(95, baseParliament + regionalVariation * 0.3 + noise * 0.2),
  );
...
export default function MinecraftFinlandMap({
  selectedBill,
  user,
}: MinecraftFinlandMapProps) {
  ...
  const districtStatsMap = useMemo(() => {
    ...
    Object.entries(DISTRICT_INFO).forEach(([code, info]) => {
      stats[code] = generateDistrictStats(
        selectedBill || null,
        code,
        info.name,
      );
  ...
  const userDistrictCode = Object.entries(DISTRICT_INFO).find(
    ([_, info]) => info.name === user?.vaalipiiri,
  )?.[0];
      ...
      <div className="flex flex-col lg:flex-row gap-12 items-start justify-center">
          ...
          <div className="grid grid-cols-7 gap-1 relative z-10">
            {FINLAND_GRID.map((row, rowIndex) =>
              row.map((cell, colIndex) => {
                ...
                const isActive =
                  hoveredCode === cell || userDistrictCode === cell;
                const agreement = stats?.agreement || 50;

                const baseColor = DISTRICT_INFO[cell].color;
                const fillColor = selectedBill
                  ? getDistrictColor(agreement, true)
                  : baseColor;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                      <span className="text-[10px] font-black text-black/10 uppercase">
                        {cell}
                      </span>
                    </div>
        ...
        <div className="w-full lg:max-w-md">
          {hoveredCode ? (
            <div className="bg-white border border-nordic-gray/30 rounded-3xl p-8 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <span className="text-[10px] font-black bg-nordic-blue/10 text-nordic-blue px-2 py-1 rounded uppercase tracking-widest mb-1 inline-block">
                    Vaalipiiri {hoveredCode}
                  </span>
              ...
              <div className="space-y-8">
                <div className="grid grid-cols-2 gap-6">
                  <div className="p-4 bg-nordic-light/50 rounded-2xl border border-nordic-gray/10">
                    <p className="text-[10px] font-bold text-nordic-dark uppercase mb-2 tracking-widest">
                      Kansan tahto
                    </p>
                    <p className="text-3xl font-black text-nordic-blue">
                      {districtStatsMap[hoveredCode].citizenVote}%
                    </p>
                  </div>
                  <div className="p-4 bg-nordic-light/50 rounded-2xl border border-nordic-gray/10">
                    <p className="text-[10px] font-bold text-nordic-dark uppercase mb-2 tracking-widest">
                    ...
                    <p className="text-3xl font-black text-nordic-deep">
                      {districtStatsMap[hoveredCode].parliamentVote}%
                    </p>
                  </div>
                </div>
                ...
                <div className="relative pt-6 border-t border-nordic-gray/20">
                  <div className="flex justify-between items-end mb-4">
                    <span className="text-sm font-bold text-nordic-darker uppercase tracking-wider">
                      Yhteensopivuus
                    </span>
                    <span
                      className={`text-5xl font-black italic leading-none ${
                        districtStatsMap[hoveredCode].agreement >= 75
                          ? "text-green-600"
                  ...
                  <div className="h-4 bg-nordic-light rounded-full overflow-hidden border border-nordic-gray/10">
                    <div
                      ...
                      style={{
                        width: `${districtStatsMap[hoveredCode].agreement}%`,
                      }}
                    />
                  </div>
                  ...
                  {districtStatsMap[hoveredCode].agreement < 50 && (
                    <div className="mt-4 p-3 bg-red-50 border border-red-100 rounded-xl flex items-center gap-3 text-red-700">
                      ...
                      <p className="text-xs font-bold leading-tight uppercase tracking-tight">
                        Merkittävä demokraattinen vaje havaittu tässä
                        vaalipiirissä
                      </p>
                    </div>
                  )}
            ...
            <div className="h-full flex flex-col items-center justify-center p-12 text-center bg-nordic-light/20 rounded-3xl border-2 border-dashed border-nordic-gray/20">
              ...
              <h4 className="text-xl font-black text-nordic-darker mb-2 uppercase tracking-tighter">
                Analysoi alueita
              </h4>
              <p className="text-sm text-nordic-dark leading-relaxed font-medium">
                Valitse vaalipiiri kartalta nähdäksesi miten alueesi mielipide
                eroaa eduskunnan päätöksenteosta.
              </p>
            </div>
          )}
          ...
          <div className="mt-8 p-6 bg-white rounded-2xl border border-nordic-gray/20 shadow-sm">
            <h5 className="text-[10px] font-black text-nordic-dark uppercase tracking-widest mb-4">
              Karttaohje
            </h5>

...
type MpRow = {
  ...
  last_name?: string;
  party?: string;
  parliament_group?: string | null;
  vaalipiiri?: string | null;
  mp_profiles?: { activity_index?: number }[];
};

export default function MpProfileMain({
  mpId,
  mp,
  activities,
  lobbying,
  ...
  /** Kun true: näytetään kansalaiskanava-ohje työpöydän kontekstissa */
      ...
      {showDashboardChrome ? (
        <div className="rounded-2xl border border-white/10 bg-slate-900/60 p-5 space-y-2">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">
            Kansalaiskanava (beta)
          </p>
          <p className="text-sm text-slate-300">
            Adressit, palaute ja kevyet yhteydenotot kootaan tänne. Syvempi
            lausuntoputki ja varjoäänestys ovat edelleen edustajan työhuoneessa.
          </p>
          <Link
            href="/dashboard?view=committee"
            className="inline-flex text-[10px] font-black uppercase tracking-widest text-purple-300 hover:text-purple-200"
          >
          ...
          <div className="space-y-4 flex-1">
            <div>
              <h1 className="text-4xl md:text-5xl font-black uppercase tracking-tighter">
                {mp.first_name}{" "}
                <span className="text-purple-500">{mp.last_name}</span>
              </h1>
            ...
            {activities && activities.length > 0 ? (
              activities.map((act) => (
                <div key={act.id} className="relative pl-12">
                  <div
                    className={`absolute left-0 top-0 w-8 h-8 rounded-full border border-white/10 flex items-center justify-center bg-slate-900 z-10 ${
                      act.activity_type === "speech"
                        ? "text-blue-400"
                        : "text-orange-400"
                    }`}
                  >
                    ...
                    {act.metadata?.location && (
                      <div className="mt-3 flex items-center gap-2 px-2 py-1 bg-rose-500/10 border border-rose-500/20 rounded-lg w-fit">
                        ...
                        <span className="text-[8px] font-black uppercase text-rose-400">
                          Local Interest: {act.metadata.location}
                        </span>
                      </div>
                    )}
                  </div>
              ...
              <p className="text-slate-500 uppercase text-[10px] font-black tracking-widest pl-12">
                Ei uusia aktiviteetteja.
              </p>
            )}
          </div>
        </div>
        ...
        <div className="space-y-12">
                ...
                {lobbying.map((org, i) => (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center text-[9px] font-bold text-slate-500">
                      <span>{org.topic}</span>
                      <span>
                        {new Date(org.date).toLocaleDateString("fi-FI")}
                      </span>
                    </div>
                  </div>
                ))}
          ...
          <section className="space-y-6">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded-md border border-amber-500/40 bg-amber-500/10 text-[9px] font-black uppercase tracking-widest text-amber-200">
                Arvio
              </span>
            </div>
            <p className="text-[11px] text-slate-500">
              Esivisualisointi: heuristinen malli, ei täyttä analyysiä ennen
              varsinaista DNA-aineistoa.
            </p>
            <div className="bg-slate-900 border border-white/5 rounded-[2.5rem] p-6">
              <div className="space-y-3">
                {["Talous", "Arvot", "Ympäristö", "Alueet", "KV", "Turva"].map(
                  (axis, i) => (

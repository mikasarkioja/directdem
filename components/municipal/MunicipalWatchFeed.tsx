...
export default function MunicipalWatchFeed({ user }: MunicipalWatchFeedProps) {
  ...
  const [selectedItem, setSelectedItem] = useState<MunicipalWatchItem | null>(
    null,
  );
          ...
          <div className="space-y-1">
            ...
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
              {selectedItem
                ...
                : "Valtuusto ja muut elimet ennen lautakuntia; listalla rajoitetusti lautakuntarivejä — Espoo, Helsinki, Vantaa"}
            </p>
          </div>
        </div>
              ...
              <div className="relative z-10 space-y-6">
                ...
                <div className="space-y-3">
                      ...
                      <div className="flex flex-col items-end gap-1">
                        <div className="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-slate-500">
                          <Flame
                            size={12}
                            className={
                              item.ai_summary.friction_index > 60
                                ? "text-orange-500"
                                : "text-slate-600"
                            }
                          />
                        ...
                        <div className="w-16 h-1.5 bg-white/5 rounded-full overflow-hidden border border-white/5">
                          <motion.div
                            initial={{ width: 0 }}
                            animate={{
                              width: `${item.ai_summary.friction_index}%`,
                            }}
                            className={`h-full ${
                              item.ai_summary.friction_index > 70
                                ? "bg-rose-500"
                                : item.ai_summary.friction_index > 40
                                  ? "bg-orange-500"
                                  : "bg-emerald-500"
                            }`}
                          />
                  ...
                  <p className="text-sm text-slate-400 leading-relaxed font-medium line-clamp-3">
                    {item.ai_summary?.summary ||
                      "Klikkaa avataksesi esitys ja generoi tekoäly-analyysi."}
                  </p>
                </div>
                      ...
                      <div key={fIdx} className="space-y-2">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-black text-white">
                            {flip.councilor_name} ({flip.councilor_party})
                          </span>
                          <div className="px-2 py-0.5 bg-rose-500 text-white text-[8px] font-black rounded uppercase">
                            {flip.axis}
                          </div>
                        </div>
                        <p className="text-xs text-rose-300/80 font-medium italic">
                          "{flip.description}"
                        </p>
                        <div className="flex items-center gap-4 pt-1">
                          <div className="space-y-1">
                            ...
                            <p className="text-xs font-black text-emerald-400">
                              {(flip.promise_score > 0 ? "+" : "") +
                                flip.promise_score.toFixed(1)}
                            </p>
                          </div>
                          ...
                          <div className="space-y-1">
                            ...
                            <p className="text-xs font-black text-rose-400">
                              {(flip.action_impact > 0 ? "+" : "") +
                                flip.action_impact.toFixed(1)}
                            </p>
                          </div>
                        </div>
                      </div>
                ...
                <div className="flex items-center justify-between pt-2">
                  <div className="flex -space-x-2">
                    {item.ai_summary.mentioned_councilors
                      ?.slice(0, 3)
                      .map((name, i) => (
                        <div
                          key={i}
                          className="w-8 h-8 bg-slate-800 border-2 border-slate-950 rounded-full flex items-center justify-center text-[10px] font-black text-slate-400 uppercase"
                          title={name}
                        >
                          {name.charAt(0)}
                        </div>
                      ))}
                    {(item.ai_summary.mentioned_councilors?.length || 0) >
                      3 && (
        ...
        {filteredItems.length === 0 && (
          <div className="text-center py-20 bg-slate-900/30 border border-dashed border-white/5 rounded-[3rem]">
            <p className="text-slate-500 font-black uppercase tracking-widest text-xs">
              Ei uusia päätöksiä tällä suodatuksella.
            </p>
          </div>
        )}
      </div>

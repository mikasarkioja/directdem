...
export async function generateMPPersona(mpId: string) {
  ...
  const votingHistoryText =
    votes
      ?.map((v) => {
        const event = Array.isArray(v.voting_events)
          ? v.voting_events[0]
          : v.voting_events;
  ...
  const { text: analysisText } = await generateText({
    ...
    system:
      "Olet poliittinen analyytikko. Tehtäväsi on luoda tiivistelmä kansanedustajan poliittisesta persoonasta perustuen hänen äänestyshistoriaansa ja profiiliinsa.",
    prompt: `
  ...
  const { error: upsertError } = await supabase.from("mp_ai_profiles").upsert({
    mp_id: mpId,

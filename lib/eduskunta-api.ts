...
function mapStatus(
  ...
  if (
    statusLower.includes("käsittelyssä") ||
...
/**
 * Normalisoi HE-tunnuksen muodon "HE 196/2025" (myös "HE196/2025", "he 196 / 2025 vp").
 */
export function normalizeHallituksenEsitysTunnus(raw: string): string {
  const blob = raw.split(",")[0].trim();
  const m = blob.match(/\bHE\s*(\d+)\s*\/\s*(\d{4})\b/i);
  if (m) {
    return `HE ${m[1]}/${m[2]}`;
  }
  return blob;
...
export type GetLatestBillsOptions = {
  /** Vaski-listaus alkaa tästä sivusta (0-pohjainen). */
  page?: number;
  /** Lisää konsoliin URL, sarakkeet ja ensimmäinen raakarivi. */
  debug?: boolean;
};

export async function getLatestBills(
  limit: number = 10,
  options?: GetLatestBillsOptions,
): Promise<EduskuntaIssue[]> {
  try {
    // Virallinen Vaski-JSON: asiakirjatyyppinimi + suodatin (Hallituksen esitys)
    // https://avoindata.eduskunta.fi/api/v1/vaski/asiakirjatyyppinimi
    const page = options?.page ?? 0;
    const perPage = Math.min(Math.max(limit, 1), 100);
    const filter = encodeURIComponent("Hallituksen esitys");
    const apiUrl = `https://avoindata.eduskunta.fi/api/v1/vaski/asiakirjatyyppinimi?perPage=${perPage}&page=${page}&languageCode=fi&filter=${filter}`;

    if (options?.debug) {
      console.log("[getLatestBills] GET", apiUrl);
    }
    ...
    const response = await fetch(apiUrl, {
      ...
      headers: {
        ...
        "User-Agent":
          "Omatase-ProcessBills/1.0 (+https://avoindata.eduskunta.fi/)",
      },
    });

    if (!response.ok) {
      throw new Error(
        `Eduskunta API unreachable: ${response.status} ${response.statusText}`,
      );
    }

    const data = await response.json();

    // Taulukkomuoto: columnNames + rowData
    const columnNames: string[] = data.columnNames || [];
    const rowData: unknown[][] = data.rowData || [];

    if (options?.debug) {
      console.log("[getLatestBills] API returned rows:", rowData.length);
      console.log("[getLatestBills] columnNames:", columnNames);
      if (rowData[0]) {
        console.log(
          "[getLatestBills] first row (raw cells):",
          JSON.stringify(rowData[0]).slice(0, 2000),
        );
      }
    }

    if (!Array.isArray(rowData) || rowData.length === 0) {
      console.warn("[getLatestBills] Eduskunta API returned no rowData");
      return [];
    }
    ...
    if (tunnusIndex < 0 || titleIndex < 0) {
      console.error(
        "[getLatestBills] Unexpected API schema — missing EduskuntaTunnus or NimekeTeksti",
      );
      return [];
    }
    ...
    for (const row of rowData) {
      ...
      const title =
        extractTextFromXML(String(titleXml)) ||
        String(titleXml)
          .replace(/<[^>]+>/g, " ")
          .trim();
      const url = extractUrl(String(urlXml));

      const tunnusPlain =
        extractTextFromXML(String(parliamentIdRaw)) ||
        String(parliamentIdRaw).trim();

      const parliamentId = normalizeHallituksenEsitysTunnus(tunnusPlain);

      if (!title && !parliamentId) continue;

      issues.push({
        id: id || parliamentId || `he-${Date.now()}-${Math.random()}`,
        parliamentId,
    ...
    if (options?.debug) {
      console.log("[getLatestBills] mapped issues count:", issues.length);
      if (issues[0]) {
        console.log(
          "[getLatestBills] first mapped issue:",
    ...
    console.warn("[getLatestBills] Failed to fetch Eduskunta data:", error);
    console.warn("[getLatestBills] Falling back to empty array.");
    return [];
  }
}
 ...
 */
export async function getBillContent(
  documentIdOrUrlRaw: string,
): Promise<string | null> {
  ...
  if (!documentIdOrUrl) {
    console.warn(
      `[getBillContent] Could not extract valid ID or URL from: ${documentIdOrUrlRaw}`,
    );
    return null;
  }
  ...
  try {
    ...
    if (documentIdOrUrl.startsWith("http")) {
      try {
        console.log(
          `[getBillContent] Attempting direct URL fetch: ${documentIdOrUrl}`,
        );
        ...
        console.log(
          `[getBillContent] Direct fetch response: ${response.status} ${response.statusText}, OK: ${response.ok}`,
        );
        ...
        if (response.ok) {
          ...
          if (
            ...
            console.log(
              `[getBillContent] Got text content: ${text.length} characters`,
            );
          ...
          if (
            ...
            documentIdOrUrl.endsWith(".pdf")
          ) {
            console.log(
              `[getBillContent] Document is a PDF, extracting text: ${documentIdOrUrl}`,
            );
            console.log(
              `[getBillContent] This may take 30-60 seconds for large PDFs...`,
            );
            try {
              const { fetchAndExtractPDF } = await import("./pdf-extractor");
              const text = await fetchAndExtractPDF(documentIdOrUrl);
              console.log(
                `[getBillContent] Successfully extracted ${text.length} characters from PDF`,
              );
              if (text && text.length > 100) {
                return text;
              } else {
                console.warn(
                  `[getBillContent] Extracted text is too short: ${text?.length || 0} characters`,
                );
              }
            } catch (pdfError: any) {
              console.error(
                `[getBillContent] Failed to extract PDF text: ${pdfError.message}`,
                pdfError,
              );
          ...
          console.warn(
            `[getBillContent] Direct URL fetch failed: ${response.status} ${response.statusText}`,
          );
        }
      } catch (err: any) {
        console.error(
          `[getBillContent] Exception during direct URL fetch: ${err.message}`,
          err,
        );
    ...
    if (!documentIdOrUrl.startsWith("http")) {
      ...
      if (s3Match) {
        ...
        console.log(
          `[getBillContent] Extracted parliament ID from S3 URL: ${parliamentId}`,
        );
      }
    }
    ...
    if (parliamentId) {
      ...
      try {
        ...
        console.log(
          `[getBillContent] S3 fetch completed. Status: ${response.status} ${response.statusText}, OK: ${response.ok}`,
        );
        ...
        if (response.ok) {
          ...
          console.log(
            `[getBillContent] Response OK. Content-Type: ${contentType}, Content-Length: ${contentLength}`,
          );
          ...
          if (contentType?.includes("pdf") || s3Url.endsWith(".pdf")) {
            console.log(
              `[getBillContent] Detected PDF, using fetchAndExtractPDF for ${s3Url}`,
            );
            try {
              const { fetchAndExtractPDF } = await import("./pdf-extractor");
              ...
              console.error(
                `[getBillContent] PDF error stack:`,
                pdfError.stack,
              );
              return null;
            }
          ...
          console.log(
            `[getBillContent] PDF detection - Content-Type: ${isPdfByContentType}, Magic Bytes: ${isPdfByMagicBytes}, Final: ${isPdf}`,
          );

          // If it's actually a PDF (detected by magic bytes), use PDF extractor
          if (isPdf) {
            console.log(
              `[getBillContent] Found PDF by magic bytes at S3 URL: ${s3Url}`,
            );
            try {
              const { fetchAndExtractPDF } = await import("./pdf-extractor");
              const text = await fetchAndExtractPDF(s3Url);
          ...
          if (
            ...
            console.log(
              `[getBillContent] Returning text content: ${text.length} characters`,
            );
            return text;
          }
          ...
          console.warn(
            `[getBillContent] Unexpected content type: ${contentType} for URL: ${s3Url}. First bytes: ${Array.from(
              firstBytes,
            )
          ...
          console.warn(
            `[getBillContent] Response not OK: ${response.status} ${response.statusText} for URL: ${s3Url}`,
          );
          const errorText = await response
            .text()
            .catch(() => "Could not read error response");
          console.warn(
            `[getBillContent] Error response body (first 500 chars): ${errorText.substring(0, 500)}`,
          );
        }
      } catch (err: any) {
        console.error(
          `[getBillContent] Exception fetching from S3 URL ${s3Url}:`,
          err,
        );
    ...
    const matchingBill = billData.find(
      (bill) =>
        bill.parliamentId === documentIdOrUrl ||
        bill.parliamentId.includes(documentIdOrUrl.split(" ")[0]),
    );

    if (matchingBill?.url) {
      try {
        console.log(
          `[getBillContent] Trying to fetch from bill URL: ${matchingBill.url}`,
        );
        ...
        if (response.ok) {
          ...
          if (contentType?.includes("application/json")) {
            ...
            const content =
              ...
              (data.rowData &&
                data.rowData[0] &&
                data.rowData[0][data.columnNames?.indexOf("Data")]) ||
              null;
            ...
            const docUrl =
              ...
              (data.rowData &&
                data.rowData[0] &&
                data.rowData[0][data.columnNames?.indexOf("Url")]);
            if (
              docUrl &&
              ...
              const urlResponse = await fetch(docUrl, {
                ...
                headers: {
                  Accept:
                    "text/html, application/xml, application/pdf, text/plain",
                  "User-Agent": "DirectDem/1.0",
                },
              });
              if (urlResponse.ok) {
                ...
                if (
                  urlContentType?.includes("text") ||
                  urlContentType?.includes("xml") ||
                  urlContentType?.includes("html")
                ) {
    ...
    console.warn(
      `[getBillContent] Could not fetch content for ${documentIdOrUrl} from any endpoint`,
    );
    return null;
  } catch (error) {
    console.error(
      `Failed to fetch bill content for ${documentIdOrUrl}:`,
      error,
    );
    return null;
  }
...
export async function getBillById(
  ...
  try {
    ...
    return {
      ...
      abstract: extractTextFromXML(
        item.abstractFi || item.abstract || "Tiivistelmä ei saatavilla.",
      ),

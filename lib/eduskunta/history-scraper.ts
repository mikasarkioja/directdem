...
export async function scrapeDependencyHistory() {
  ...
  for (const mp of mps) {
    console.log(
      `   Processing history for ${mp.first_name} ${mp.last_name}...`,
    );
    ...
    try {
      ...
      console.log(
        `   Found ${declarations.length} potential declarations for ${mp.last_name}.`,
      );
      ...
      for (const row of declarations) {
        ...
        $("sis\\:KappaleKooste, KappaleKooste").each((i, el) => {
          ...
          if (
            text.length > 10 &&
            (text.includes("hallitus") || text.includes("omistus"))
          ) {
            itemsInVersion.push({
              category: text.includes("hallitus")
                ? "Hallitustyöskentely"
                : "Omistus",
              description: text,

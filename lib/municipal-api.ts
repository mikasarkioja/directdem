...
export class HelsinkiAPI extends MunicipalAPI {
  ...
  async fetchLatestItems(limit: number = 10): Promise<MunicipalAgendaItem[]> {
    console.log(
      `[HelsinkiAPI] Fetching latest items from Helsinki, limit: ${limit}`,
    );
    ...
    for (const url of endpoints) {
      try {
        ...
        if (response.ok) {
          ...
          const objects =
            data.objects || data.items || (Array.isArray(data) ? data : []);

          if (objects.length > 0) {
            console.log(
              `[HelsinkiAPI] Successfully fetched ${objects.length} items from ${url}`,
            );
          ...
          console.warn(
            `[HelsinkiAPI] Endpoint ${url} returned status ${response.status}`,
          );
        }
      } catch (error: any) {
  ...
  private getMockItems(): MunicipalAgendaItem[] {
    console.log(
      "[HelsinkiAPI] Returning realistic Helsinki mock items as fallback",
    );
    return [
      {
        id: "mock-hel-1",
        ...
        summary:
          "Kaupunkiympäristölautakunta käsittelee historiallisen puiston kunnostussuunnitelmaa.",
        content:
          "Suunnitelma sisältää uusien valaisimien ja penkkien asentamisen...",
        status: "agenda",
      ...
      },
      {
        ...
        summary:
          "Päätös uusista pyöräilyreiteistä ja jalankulun sujuvoittamisesta keskustassa.",
...
export class KuntalaisaloiteAPI extends MunicipalAPI {
  ...
  async fetchLatestItems(limit: number = 10): Promise<MunicipalAgendaItem[]> {
    try {
      ...
      if (!response.ok)
        throw new Error(`Kuntalaisaloite API error: ${response.status}`);
    ...
    return [
      {
        ...
        summary:
          "Vantaan kaupunginvaltuusto hyväksyi hankesuunnitelman uudesta kampusalueesta.",
      ...
      },
      {
        ...
        summary:
          "Keskustelu ratikan itäisen jatkeen vaikutuksista asuinalueisiin.",

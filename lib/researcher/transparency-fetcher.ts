...
export async function syncTransparencyRegister() {
  ...
  try {
    ...
    const mockApiData = [
      { organization: "Elinkeinoelämän keskusliitto EK", lobbyist: "Jyri Häkämies", target: "Petteri Orpo", topic: "Työmarkkinauudistus ja vientimalli", date: "2025-12-10" },
      { organization: "SAK", lobbyist: "Jarkko Eloranta", target: "Li Andersson", topic: "Työttömyysturvan suojaosat", date: "2025-12-05" },
      { organization: "Metsäteollisuus ry", lobbyist: "Paula Lehtomäki", target: "Sanni Grahn-Laasonen", topic: "TKI-rahoitus ja puun saatavuus", date: "2025-11-28" },
      { organization: "Finnish Energy", lobbyist: "Jukka Leskelä", target: "Harry Harkimo", topic: "Sähkömarkkinalaki", date: "2025-12-12" }
    ];
    ...
    for (const item of mockApiData) {
      ...
      if (mp) {
        ...
        console.log(`   ⚠️ Could not find MP matching ${item.target} (${lastName})`);
      }
    }

    console.log(`✨ Sync Complete! Imported ${newMeetingsCount} real-world lobbying activities.`);
